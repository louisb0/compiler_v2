...
public:
    Binary(Token op_symbol, std::unique_ptr<Expression> left_node, std::unique_ptr<Expression> right_node)
        : op_symbol(op_symbol), left_node(std::move(left_node)), right_node(std::move(right_node)) {};

    const Token symbol() const { return op_symbol; }
...
public:
    Unary(Token op_symbol, std::unique_ptr<Expression> expr) : op_symbol(op_symbol), expr(std::move(expr)) {}
